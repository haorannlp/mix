...
@register_criterion('seq_risk_loss')
class MySequenceRiskCriterion(FairseqCriterion):
    ...
    def __init__(self, task, sentence_avg, label_smoothing):
        ...
        self.alpha = 0.001
        self.scale_scores = True

        search_strategy = search.LengthConstrainedBeamSearch(task.target_dictionary, min_len_a=0,
                                                             min_len_b=1,
                                                             max_len_a=1.2,
                                                             max_len_b=5, )
        self._generator = SequenceGenerator(task.target_dictionary, beam_size=4, match_source_len=False,
    ...
    def sequence_forward(self, net_output, model, sample):
        ...
        #probs = (avg_scores.exp()).div(torch.sum(avg_scores.exp(), dim=-1).unsqueeze(1))
        assert probs.dim() == avg_scores.dim()
        ...
        if torch.any(torch.isnan(loss)) or torch.any(torch.isnan(avg_scores)):
            print(loss)
        sample_size = sample['ntokens']    #net_output.size(0)       #!!!!!!!!!!!!!!!!!
        logging_output = {
            'loss': loss.data,
