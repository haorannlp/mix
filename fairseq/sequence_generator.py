...
class SequenceGenerator(object):
    def __init__(
        ...
        self.match_target_len = match_target_len
    ...
    def _generate(
        ...
        # src_tokens: batch_size x max_len
        # src_lengths: batch_size
        # tgt_lengths: batch_size
        src_tokens = encoder_input['src_tokens']
        src_lengths = (src_tokens.ne(self.eos) & src_tokens.ne(self.pad)).long().sum(dim=1)
        target = sample.get('target', None) if target is None else target
        tgt_lengths = (target.ne(self.eos) & target.ne(self.pad)).long().sum(
        ...
        # encoder_outs: List
        # encoder_outs[0].encoder_embedding : batch_size x max_len x emb_size
        # encoder_outs[0].encoder_out: max_len x batch_size x hidden_size (hidden_size==emb_size)
        # encoder_outs[0].encoder_padding_mask: batch_size x max_len
        encoder_outs = model.forward_encoder(encoder_input)
        #original_encoder_outs = encoder_outs[0].encoder_out.clone()                                          ### modif
        new_order = torch.arange(bsz).view(-1, 1).repeat(1, beam_size).view(-1)
        new_order = new_order.to(src_tokens.device).long()
        encoder_outs = model.reorder_encoder_out(encoder_outs, new_order)
        # encoder_outs[0].encoder_padding_mask: batch_size*beam_size x max_len
        # encoder_outs[0].encoder_out: max_len x batch_size*beam_size x hidden_size
        # initialize buffers
        #
        ...
        def finalize_hypos(step, bbsz_idx, eos_scores):
            ...
            for i, (idx, score) in enumerate(zip(bbsz_idx.tolist(), eos_scores.tolist())):
                ...
                if self.match_source_len and step > src_lengths[unfin_idx]:
                    score = -math.inf
                elif self.match_target_len and step > tgt_lengths[unfin_idx]:
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            # lprobs: batch_size*beam_size x vocab_size
            ...
            if step >= max_len:
                orig_lprobs = lprobs.clone().detach()
            ...
            if tgt_lengths is not None:
                self.search.set_src_lengths(tgt_lengths)
            else:
            ...
            # if torch.any(torch.eq(torch.sum(lprobs.exp(), dim=-1), 0)):
            #     print('step:',step, max_len)
            #     print('how?')
            if torch.any(torch.eq(torch.sum(lprobs.exp(), dim=-1), 0)):
                only_eos = -200 * torch.ones(lprobs.size(-1), device=lprobs.device)
                only_eos[self.eos] = 1.0
                rows = torch.eq(torch.sum(lprobs.exp(), dim=-1), 0)
                lprobs[rows] = only_eos
            ...
            if num_remaining_sent == 0:
                break
            assert step < max_len, (step, max_len)
            ...
            if len(finalized_sents) > 0:
                ...
                if tgt_lengths is not None:
                    tgt_lengths = tgt_lengths[batch_idxs]
...
class EnsembleModel(torch.nn.Module):
    ...
    def _decode_one(
        ...
        dis_prob = probs.exp()
        # assert dis_prob.sum().eq(torch.tensor([1.0]))
        entropy = torch.distributions.Categorical(dis_prob).entropy()
